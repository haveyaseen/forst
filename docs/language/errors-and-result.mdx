...
like other Results (`ensure r is Ok()`) or keep Error-shaped `ensure !err` when
the void Result is folded to a single error name.
...
`ensure !err`. Use `if x is Ok()` when you want local handling without
promotion.
