...
TypeScript **`_tag`** export for named Forst errors is still maturing. Treat those payloads as evolving. Invoke transport failures (`InvokeRejected`, …) already use stable tags. See [Return a named error § Caveats](/language/named-errors#caveats) and [Call Forst over HTTP](/interop/invoke/call-forst#failures).
